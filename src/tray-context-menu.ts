                ...
                },
                {
                    ...
                    click: async () => {
                        ...
                        await options.withBrowserWindow(
                            ...
                            {
                                onFailureMessage: '无法重新加载窗口，请稍后重试'
                            }
                        )
                    }
                },
                    ...
                    submenu: [
                        {
                            ...
                            click: () => {
                                ...
                                const savedUrl =
                                    newUserSetting.urls?.ChatGPT || urls.chatgpt
                        ...
                        { type: 'separator' },
                        {
                            ...
                            click: () => {
                                ...
                                tray.popUpContextMenu(options.menu)
                                const savedUrl =
                                    newUserSetting.urls?.DeepSeek || urls.deepseek
                                getAvailableBrowserWindow(
                                    electronMenubar,
                                    options.getMainBrowserWindow
                                )
                    ...
                    click: async () => {
                        ...
                        try {
                            ...
                            if (!options.isMenubarReady()) {
                                ...
                                if (!options.isMenubarReady()) {
                                    console.log('⚠️ Menubar 尚未 ready，但继续尝试...')
                                }
                            }
                            ...
                            console.log('🔍 初始窗口状态:', {
                                ...
                                mainBrowserWindow: !!options.getMainBrowserWindow(),
                                electronMenubarBrowserWindow: !!electronMenubar.browserWindow,
                            ...
                            if (!browserWindow || browserWindow.isDestroyed()) {
                                console.log('📦 窗口不存在或已销毁，创建窗口...')
                                try {
                                    if (!electronMenubar.tray) {
                                        console.error('❌ Tray 未初始化，无法创建窗口')
                                        dialog.showMessageBox({
                                            type: 'error',
                                            title: '错误',
                                            message: '应用程序未完全初始化，请稍后再试',
                                            buttons: ['确定']
                                        })
                                        return
                                    }
                                    ...
                                    for (let i = 0; i < 5; i++) {
                                        ...
                                        if (
                                            ...
                                            console.log(`✅ 窗口获取成功 (尝试 ${i + 1}/5)`)
                                            break
                                        }
                                        console.log(`⏳ 等待窗口创建... (尝试 ${i + 1}/5)`)
                                        await delay(100)
                                    }
                                    ...
                                    if (
                                        ...
                                        options.setMainBrowserWindow(browserWindow)
                                        console.log('✅ 窗口引用已更新')
                                    }
                                } catch (error) {
                            ...
                            if (input && input.trim()) {
                                ...
                                const existingShortcut = options.getCurrentShortcut()
                                if (existingShortcut) {
                                ...
                                if (registered) {
                                    ...
                                    if (existingShortcut) {
                                        globalShortcut.register(existingShortcut, () => {
                                            const menubarWindow =
                                                getAvailableBrowserWindow(
                                                    electronMenubar,
                                                    options.getMainBrowserWindow
                                ...
                                if (resetResult.response === 0) {
                                    const currentShortcut = options.getCurrentShortcut()
                                    if (currentShortcut) {
                                    ...
                                    if (defaultRegistered) {
                                        ...
                                        dialog.showMessageBox(browserWindow, {
                                            type: 'info',
                                            title: '设置成功',
                                            message: '快捷键已重置为默认值: CommandOrControl+g',
                                            buttons: ['确定']
                                        })
                                    } else {
                                        dialog.showMessageBox(browserWindow, {
